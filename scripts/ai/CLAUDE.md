...
- `die()` stops the AI and animation, disables the capsule collider, and calls `skeleton.physical_bones_start_simulation()` on whatever `Skeleton3D` it finds inside the model — see the ragdoll setup step below, **this does nothing until that's done**. Applies the killing hit's impulse to whichever `PhysicalBone3D` looks like the torso/hips (falls back to the first one found). Corpses `queue_free()` after `corpse_lifetime` seconds (default 20).
