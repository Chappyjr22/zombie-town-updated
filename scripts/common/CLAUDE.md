...
- `physics_layers.gd` (`class_name PhysicsLayers`) — named constants for the collision layer bit values (`WORLD`, `ACTORS`, `RAGDOLL`, `HITBOX`). `scripts/player/player.gd` and `scripts/ai/zombie.gd` set their `collision_layer`/`collision_mask` from these instead of hardcoding the numbers, so the layers stay consistent project-wide. See the file's own comments for why ragdolls and hitboxes each need their own layer separate from living actors.
