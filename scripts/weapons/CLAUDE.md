...
- **The weapon sits at an angle to the crosshair.** `_align_world_model()` lays the weapon along the line between the body's hands, so it points where the animation holds it. A spine-yaw correction was tried and reverted — it measured the barrel as the model's local `-Z`, which stops being the barrel once `_align_world_model()` has rotated it. Fix that measurement first if revisiting. Shots are unaffected: the trace comes off the camera ray.
